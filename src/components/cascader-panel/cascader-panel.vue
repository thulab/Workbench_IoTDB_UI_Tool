...
import { checkNode, getMenuIndex, sortByOriginalOrder } from 'element-plus/es/components/cascader-panel/src/utils';
import { CommonProps, useCascaderConfig } from 'element-plus/es/components/cascader-panel/src/config';
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const updateKeyChildren = (node: Node) => {
      const cfg = config.value;
      node! = node || new Node({}, cfg, undefined, true);
      const _node = node as Node;
      const parent = _node.root ? _node : _node.parent;
      parent.loading = true;
      parent.loaded = false;
      return (dataList: CascaderOption[]) => {
        store?.cleanChildNodes(parent);
        if (dataList.length) {
          store?.appendNodes(dataList, parent as any);
        }
    ...
    const getCheckedNodes = (leafOnly: boolean) => getFlattedNodes(leafOnly)?.filter((node) => node.checked !== false);
