<template>
  ...
  <el-container class="data-trend-wrapper">
    <el-header class="p-0" style="height: auto">
      <div class="search-form-wrapper search-form-box">
        ...
        <div class="search-form-buttons">
          ...
          <el-button :disabled="!searchFormData.selectedMeasurement.length" @click="handleSaveTemplate" id="trend-search-save-template">{{ t('common.save') }}</el-button>
        </div>
      </div>
        ...
        <el-aside :width="isExpand ? '240px' : '24px'" :class="['path-list-wrapper', !isExpand ? 'p-0' : '']">
          <el-tabs v-model="activeNameSide" stretch class="tabs-nav-aside" v-if="isExpand">
            ...
            <el-tab-pane :label="t('dataTrend.commonTemplates')" name="template">
              <template-list-tab ref="templateListRef" sql-dialect="table" @handle-operate="handleOperateTemplate" />
            </el-tab-pane>
          </el-tabs>
    ...
    <modal-table-measurement
      ...
      :selected-measurements="searchFormData.selectedMeasurement"
...
import { SearchApi, TableDataApi } from '@/api';
...
const aggregateFunctions = computed(() => [
  { label: t('common.lastValue'), value: 'last' },
  { label: t('common.maxValue'), value: 'max' },
  { label: t('common.minValue'), value: 'min' },
...
const applyTip = computed(() => {
  ...
  if (searchFormData.selectedMeasurement.length > 10) {
...
function handleSearch(unforce?: boolean) {
  if (!searchFormData.selectedMeasurement.length) return;
  if (isRunningTab.value) {
    if (!unforce) {
      dealSearchPath();
    } else {
  ...
  getHistoryTrend({
    ...
    pathList.value.forEach((item) => {
      if (abnormals.includes(item.path)) {
        item.disabled = true;
        item.checked = false;
      } else {
        item.disabled = false;
...
function handleOperateTemplate(val: string, data: Search.TrendTemplate) {
  if (val === 'rename') {
    ...
    searchFormData.database = templateData.database;
    searchFormData.table = templateData.table;
    searchFormData.selectedMeasurement = templateData.selectedMeasurement;
