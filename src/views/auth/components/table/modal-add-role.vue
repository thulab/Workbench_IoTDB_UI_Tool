  ...
  <el-dialog :title="t('auth.relateRole')" v-model="dialogVisible" width="480px" class="add-role-modal" align-center :close-on-click-modal="false" id="auth-user-add-role-modal">
    ...
    <template #footer>
      <div class="dialog-footer m-t-34">
        ...
        <el-button type="primary" :disabled="formData.name.length === 0" :loading="grantLoading" @click="handleConfirm" id="auth-user-add-role-modal-confirm">{{ t('common.confirm') }}</el-button>
      </div>
    </template>
  </el-dialog>
</template>
...
const { requestFn: getList, loading } = useRequest(RelationalPrivilegesApi.getRoles);
