  ...
  <el-dialog :title="t('auth.relateUser')" v-model="dialogVisible" width="480px" class="add-user-modal" align-center :close-on-click-modal="false" id="auth-role-add-user-modal">
    ...
    <template #footer>
      <div class="dialog-footer m-t-34">
        ...
        <el-button type="primary" :disabled="formData.name.length === 0" :loading="grantLoading" @click="handleConfirm" id="auth-role-add-user-modal-confirm">{{ t('common.confirm') }}</el-button>
      </div>
    </template>
  </el-dialog>
</template>
...
const { requestFn: getList, loading } = useRequest(RelationalPrivilegesApi.getUsers);
const { requestFn: grantRolesToUser, loading: grantLoading } = useRequest(RelationalPrivilegesApi.grantRolesToUser);
...
function isCanSelect(item: string) {
  ...
  return props.selected.includes(item) || item === 'root';
}
