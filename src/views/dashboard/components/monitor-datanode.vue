  ...
  <div class="monitor-chart-wrapper">
    ...
    <div class="monitor-chart-box-media" v-loading="diskLoading">
      <div class="monitor-chart-container">
        ...
        <data-container :is-empty="diskMemoryData.diskTotal === null" :style="{ transform: diskMemoryData.diskTotal === null ? 'translateY(-8px)' : '' }">
...
function getDisk() {
  return getMetricDisk(props.node, isMaster.value)
    .then((res) => {
...
function getFile() {
  return getMetricFileCount(props.node, props.nodeType, isMaster.value)
    .then((res) => {
...
function getInitial() {
  Promise.allSettled([getCpu(), getCpuLoad(), getDisk(), getMemory(), getFile(), getIo()]).then(() => {
    emit('handleFetch');
  });
}
