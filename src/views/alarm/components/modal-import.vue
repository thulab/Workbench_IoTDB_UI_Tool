<template>
  <el-dialog :title="t('alarm.batchImportAlarm')" v-model="dialogVisible" width="748px" class="new-storage-container" align-center :close-on-click-modal="false" id="alarm-modal-import">
    <div class="import-box">
      <el-steps :active="activeStep" align-center finish-status="success" class="import-step-box" id="alarm-import-steps">
    ...
    <div class="operate-buttons m-t-12" style="text-align: right" v-if="activeStep === 0">
      <el-button type="primary" :disabled="!uploadFileInfo" @click="handleNext" id="alarm-import-next">{{ t('common.next') }}</el-button>
    </div>
    ...
    <div class="operate-buttons m-t-12" style="text-align: center" v-if="activeStep === 2">
      <el-button type="primary" @click="handleClose" id="alarm-import-close">{{ t('common.finish') }}</el-button>
    </div>
  </el-dialog>
</template>
...
const { requestFn: importAlarmData } = useRequest(AlarmApi.importAlarmData);
...
const customUpload: UploadProps['httpRequest'] = (options) => {
  ...
  return importAlarmData(formData, isCSV.value ? 'csv' : 'xlsx')
    .then((res) => {
      if (res.code === 0) {
