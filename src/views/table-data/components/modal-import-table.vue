<template>
  <el-dialog
    :title="`${importType === 'table' ? t('dataManage.batchImportTable') : t('dataManage.batchImportColumn')}`"
    v-model="dialogVisible"
    width="748px"
    class="new-storage-container"
    align-center
    :close-on-click-modal="false"
    id="table-data-modal-import"
  >
    <div class="import-box">
      <el-steps :active="activeStep" align-center finish-status="success" class="import-step-box" id="table-data-import-steps">
      ...
      <div class="select-file-box" v-if="activeStep === 0">
        <div class="select-item-box" style="align-items: center">
          ...
          <a :href="`/api/file/exportTableColumnTemplateTable?database=${currentNode.database}`" class="template-button" target="_blank" rel="noopener noreferrer">table_template.csv</a>
        </div>
    ...
    <div class="m-t-12 operate-buttons" style="text-align: right" v-if="activeStep === 0">
      <el-button type="primary" :disabled="!uploadFileInfo" @click="handleNext" id="table-data-import-next">{{ t('common.next') }}</el-button>
    </div>
    ...
    <div class="m-t-12 operate-buttons" style="text-align: center" v-if="activeStep === 2">
      <el-button type="primary" @click="handleClose" id="table-data-import-close">{{ t('common.finish') }}</el-button>
    </div>
  </el-dialog>
</template>
...
const { requestFn: importTables } = useRequest(IoTDBApi.importTables);
...
const customUpload: UploadProps['httpRequest'] = (options) => {
  ...
  return importTables(formData, isCSV.value ? 'csv' : 'xlsx')
    .then((res) => {
      if (res.code === 0) {
