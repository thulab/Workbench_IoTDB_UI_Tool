<template>
  ...
  >
    <div class="import-box">
      ...
      <div class="select-file-box" v-if="activeStep === 0">
        <div class="select-item-box" style="align-items: center">
          ...
          <a href="/api/file/exportCalculateTemplate" class="template-button" target="_blank">{{ appType === 1 ? 'calculation_template.csv' : 'view_template.csv' }}</a>
        </div>
    ...
    <div class="m-t-12 operate-buttons" style="text-align: right" v-if="activeStep === 0">
      <el-button type="primary" :disabled="!uploadFileInfo" @click="handleNext" id="calculate-import-next">{{ t('common.next') }}</el-button>
    </div>
...
const { requestFn: importCalculateData } = useRequest(CalculateApi.importCalculateData);
...
const customUpload: UploadProps['httpRequest'] = (options) => {
  ...
  return importCalculateData(formData, isCSV.value ? 'csv' : 'xlsx')
    .then((res) => {
      if (res.code === 0) {
