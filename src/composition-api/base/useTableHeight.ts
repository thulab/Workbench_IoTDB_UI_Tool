...
export default function useTableHeight(overflowMinHeight: number, overflowOtherHeight?: number) {
  ...
  const maxTableHeight = computed(() => (overflowOtherHeight && innerHeight.value > 900 ? Calculator.subtract(maxTableHeightOverflowMin.value, overflowOtherHeight) : maxTableHeightOverflowMin.value));
